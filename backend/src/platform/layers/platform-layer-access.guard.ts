...
export class PlatformLayerAccessGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const requiredLayer = routeLayer(String(request.path || request.url || ''));
    ...
    // Existing installations remain operational until DEV explicitly configures
    // their layers. From the first selection onward every excluded route is off.
