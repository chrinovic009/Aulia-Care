...
const layersForPath = (pathname: string): AuliaLayer[] | null => {
  ...
  if (["/montre-connectee", "/enfants", "/reception/montres", "/administration/montres", "/connected-care"].some((path) => pathname.startsWith(path))) return ["CONNECTED"];
  if (["/suivi-quotidien", "/telehealth", "/teleconsultation", "/clinical-intelligence", "/intelligence"].some((path) => pathname.startsWith(path))) return ["AI"];
  return ["CORE"];
};
