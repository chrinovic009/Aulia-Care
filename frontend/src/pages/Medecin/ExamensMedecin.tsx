...
const getLabRequestViewState = (
  ...
  const hasResults = Boolean(
    request.results?.some((result) => (result.resultValue || "").trim()),
  );
  ...
  if (availableStatuses.has(normalizedRequestStatus)) {
    return {
      ...
      message: hasResults
        ...
        : "Cet examen a déjà été traité.",
      showResults: hasResults,
    };
  }

  if (
    normalizedWorkflow === "EN_ATTENTE_VALIDATION_CAISSE" ||
    normalizedWorkflow === "EN_ATTENTE_DE_PAIEMENT"
  ) {
    return {
      ...
      message:
        "Le paiement doit être validé par la caisse avant que le laboratoire ne puisse traiter cet examen.",
      showResults: false,
    };
  }
  ...
  return {
    ...
    message:
      "L'examen a été transmis au laboratoire et est en cours de traitement.",
    showResults: false,
  };
};
...
const formatLabResultParameter = (parameter: LabResultParameter) => {
  ...
  const value =
    parameter.valueNumeric?.toString() ||
    parameter.valueText ||
    "Non renseigné";
  const unit = parameter.labTestParameter?.unit?.trim() || "";
  const reference = parameter.labTestParameter?.referenceRange?.trim() || "";
  const interpretation = parameter.interpretation
    ? ` • ${parameter.interpretation}`
    : "";
  const referenceText = reference
    ? ` | Réf: ${reference}${unit && !reference.toLowerCase().includes(unit.toLowerCase()) ? ` ${unit}` : ""}`
    : unit
      ? ` ${unit}`
      : "";
...
const formatLabResultTextWithReference = (result: DoctorLabResult) => {
  if (Array.isArray(result.parameters) && result.parameters.length > 0) {
    return result.parameters
      .map((parameter) => formatLabResultParameter(parameter))
      .join("\n");
  }
  ...
  const hasUnitAlready = Boolean(
    units && reference.toLowerCase().includes(units.toLowerCase()),
  );
...
const isNfsExam = (name?: string | null) =>
  Boolean(
    /(^|\s)(nfs|h[eé]mogramme|num[eé]ration formule sanguine)(\s|$)/i.test(
      name || "",
    ),
  );
...
export default function ExamensMedecin() {
  ...
  >([]);
  const [imagingCatalogue, setImagingCatalogue] = useState<
    ImagingCatalogueItem[]
  >([]);
  ...
  const [serviceUnits, setServiceUnits] = useState<
    Array<{ id: string; name: string; departmentId?: string }>
  >([]);
  ...
  const load = async () => {
    const [
      ...
      apiFetch<Array<{ id: string; name: string; isParamedical?: boolean }>>(
        "/administration/departments",
      ).catch(() => []),
      apiFetch<Array<{ id: string; name: string; departmentId?: string }>>(
    ...
    setSelectedPatientId(
      (current) => current || withConsultations[0]?.id || "",
    );
    setSelectedConsultationId(
      (current) =>
        current || withConsultations[0]?.consultations?.[0]?.id || "",
    );
  };
  ...
  const filteredPatients = useMemo(() => {
    ...
    return patients.filter(
      (patient) =>
        !normalized || patientSearchText(patient).includes(normalized),
    );
  }, [patients, query]);
  ...
  const selectedConsultation =
    selectedPatient?.consultations?.find(
  ...
  const selectedService =
    serviceUnits.find((s) => s.id === form.serviceId) ||
    services.find((service) => service.id === form.serviceId);
  const isDepartmentLaboratory = Boolean(
    departments.find(
      (d) => d.id === form.departmentId && d.name === "Laboratoire Medical",
    ),
  );
  const selectedLabTests = useMemo(
    () => labTests.filter((test) => selectedLabTestIds.includes(test.id)),
    [labTests, selectedLabTestIds],
  );
  const selectedImagingItem =
    imagingCatalogue.find((item) => item.id === form.imagingCatalogueId) ||
    null;
  const paramedicalDepartments = useMemo(
    () =>
      departments.filter(
        (d) => d.isParamedical && !/pharmacie|pharmacy/i.test(d.name || ""),
      ),
    [departments],
  );

  const submit = async () => {
    const isImagingRequest = Boolean(
      form.imagingCatalogueId || form.examName.trim(),
    );
    const isLabRequest = isDepartmentLaboratory
      ? selectedLabTestIds.length > 0
      : Boolean(form.labTestId || form.examName.trim());

    if (!selectedConsultation || (!isLabRequest && !isImagingRequest)) {
      setMessage(
        "Choisissez une consultation et renseignez au moins un examen.",
      );
      return;
    }
    if (!canWrite) {
      setMessage(
        "Dossier en lecture seule: seul le medecin autorise peut demander un examen.",
      );
      return;
    }
    ...
    if (form.imagingCatalogueId) {
      await createImagingRequest(selectedConsultation.id, {
        ...
        selectedIncidences: form.selectedIncidences
          .split(",")
          .map((item) => item.trim())
          .filter(Boolean),
      ...
      const labTestIds = isDepartmentLaboratory
        ? selectedLabTestIds
        : form.labTestId
          ? [form.labTestId]
          : [];
      ...
      await createLabRequest(selectedConsultation.id, {
        ...
        examName:
          selectedLabTests.length > 0
            ? selectedLabTests.map((test) => test.name).join(", ")
            : form.examName,
        specimenType:
          selectedLabTests.length > 0
            ? selectedLabTests.map((test) => test.name).join(", ")
            : form.specimenType || selectedService?.name || form.examName,
        notes: [
          form.notes,
          selectedService ? `Service paramedical: ${selectedService.name}` : "",
          selectedLabTests.length > 0
            ? `Examens catalogue: ${selectedLabTests.map((test) => test.name).join(", ")}`
            : "",
        ]
          .filter(Boolean)
          .join("\n"),
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 dark:bg-slate-950 sm:p-6">
      <PageMeta
        title="Examens medecin | Aulia Care"
        description="Demandes et resultats d'examens."
      />
      <PageBreadcrumb pageTitle="Examens demandes" />
      <Header
        title="Examens complementaires"
        subtitle="Choisir une consultation, demander un examen, puis suivre les resultats."
      />
      {patients.length === 0 ? (
        <EmptyState />
      ) : (
                ...
                <div className="mt-5 grid gap-4 xl:grid-cols-2">
                  ...
                  {selectedConsultation?.id === "__legacy_exam_panel_disabled__" && <Panel title="Nouvelle demande">
                    <Select
                      ...
                      options={(selectedPatient.consultations || []).map(
                        (consultation) =>
                          [
                            consultation.id,
                      ...
                      >
                        ...
                        {imagingCatalogue.map((item) => (
                          <option
                            key={item.id}
                            value={item.id}
                          >{`${item.name} (${item.modality}) - ${Number(item.price || 0).toLocaleString("fr-FR")} CDF`}</option>
                        ))}
                      </select>
                    </label>
                    {selectedImagingItem ? (
                      <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm dark:border-slate-800 dark:bg-slate-950">
                        <p className="font-semibold text-slate-900 dark:text-white">
                          {selectedImagingItem.name}
                        </p>
                        <p className="text-xs text-slate-500">
                          {selectedImagingItem.description ||
                            "Aucune description disponible."}
                        </p>
                        <p className="mt-2 text-xs text-slate-500">
                          Modalité: {selectedImagingItem.modality}
                        </p>
                        <p className="text-xs text-slate-500">
                          Catégorie: {selectedImagingItem.category || "Général"}
                        </p>
                        <p className="text-xs text-slate-500">
                          Incidences possibles:{" "}
                          {selectedImagingItem.availableIncidences.join(", ") ||
                            "Aucune"}
                        </p>
                        <p className="text-xs text-slate-500">
                          Contraste autorisé:{" "}
                          {selectedImagingItem.supportsContrast ? "Oui" : "Non"}
                        </p>
                      </div>
                    ) : null}
                    {selectedImagingItem ? (
                      <>
                        ...
                        <label className="block text-sm">
                          ...
                          <input
                            ...
                            onChange={(event) =>
                              setForm((current) => ({
                                ...current,
                                selectedIncidences: event.target.value,
                              }))
                            }
                        ...
                        <label className="flex items-center gap-3 text-sm">
                          <input
                            ...
                            onChange={(event) =>
                              setForm((current) => ({
                                ...current,
                                contrastAgentUsed: event.target.checked,
                              }))
                            }
                        ...
                        {form.contrastAgentUsed ? (
                          <Textarea
                            label="Détails contraste"
                            value={form.contrastDetails}
                            onChange={(value) =>
                              setForm((current) => ({
                                ...current,
                                contrastDetails: value,
                              }))
                            }
                          />
                        ) : null}
                        <Textarea
                          label="Protocole / remarques"
                    ...
                    {isDepartmentLaboratory ? (
                      <div className="space-y-2">
                        <label className="block text-sm">
                          <span className="mb-1 block font-medium text-slate-600 dark:text-slate-300">
                            Examen du catalogue laboratoire
                          </span>
                          <select
                            value=""
                            onChange={(event) => {
                          ...
                          >
                            ...
                            {labTests.map((test) => (
                              <option
                                key={test.id}
                                value={test.id}
                              >{`${test.name} - ${Number(test.price || 0).toLocaleString("fr-FR")} CDF - ${test.turnaroundTimeMinutes || "-"} min`}</option>
                            ))}
                          </select>
                        </label>
                        {selectedLabTests.length > 0 ? (
                          <div className="space-y-2 rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm dark:border-slate-800 dark:bg-slate-950">
                            {selectedLabTests.map((test) => (
                              <div
                                key={test.id}
                                className="flex items-center justify-between gap-3 rounded-lg bg-white px-3 py-2 dark:bg-slate-900"
                              >
                                <div>
                      ...
                      <Select
                        ...
                        onChange={(value) =>
                          setForm((current) => ({
                            ...
                            serviceId: value,
                            examName:
                              serviceUnits.find((s) => s.id === value)?.name ||
                              current.examName,
                          }))
                        }
                        options={[
                      ...
                      <div className="rounded-lg border border-blue-100 bg-blue-50 p-3 text-xs text-blue-800">
                        {selectedLabTests.map((test) => (
                          <div key={test.id} className="mb-2 last:mb-0">
                            ...
                            Section: {test.section?.name || "-"} | Catégorie:{" "}
                            {test.category?.name || "-"} | Prix:{" "}
                            {Number(test.price || 0).toLocaleString("fr-FR")}{" "}
                            CDF | Délai: {test.turnaroundTimeMinutes || "-"} min
                          </div>
                        ))}
                      </div>
                    ) : null}
                    ...
                    <button
                      disabled={!canWrite}
                      onClick={submit}
                      className="rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white disabled:bg-slate-300 disabled:text-slate-600"
                    >
                      Envoyer la demande
                    </button>
                  </Panel>}
                      ...
                      <div className="space-y-4">
                              ...
                              >
                                ...
                                {request.report?.impression ? (
                                  <p className="mt-3 text-slate-600 dark:text-slate-300">
                                    Impression: {request.report.impression}
                                  </p>
                                ) : (
                                  <p className="mt-3 text-slate-600 dark:text-slate-300">
                        ...
                        {recentLabRequests.length > 0 ? (
                          <div className="space-y-3">
                            <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm dark:border-slate-800 dark:bg-slate-950">
                              <p className="font-semibold text-slate-900 dark:text-white">
                                Demandes de laboratoire récentes
                              </p>
                            </div>
                            {recentLabRequests.map((request) => {
                              const viewState = getLabRequestViewState(
                                request,
                                selectedPatient.workflowStatus,
                              );
                              return (
                                <div
                                  key={request.id}
                                ...
                                >
                                  <div className="flex flex-wrap items-start justify-between gap-2">
                                    ...
                                    <span
                                      className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${viewState.badgeClassName}`}
                                    >
                                  ...
                                  {viewState.showResults ? (
                                    <div className="mt-3 space-y-2">
                                      {(() => {
                                        const examName =
                                          (request as any).examName ||
                                          request.specimenType ||
                                          "";
                                        const isNfsRequest =
                                          isNfsExam(examName) ||
                                          Boolean(
                                            request.results?.some(
                                              (r: any) =>
                                        ...
                                        if (isNfsRequest) {
                                          ...
                                          return params.map(
                                            (p: any, idx: number) => (
                                              <div
                                                key={`${request.id}-param-${idx}`}
                                                className="rounded-lg border border-slate-200 bg-white/80 p-2.5 dark:border-slate-800 dark:bg-slate-900/70"
                                              >
                                                <p className="font-medium text-slate-700 dark:text-slate-200 whitespace-pre-line">
                                                  {formatLabResultParameter(p)}
                                                </p>
                                              </div>
                                            ),
                                          );
                                        ...
                                        return request.results?.map(
                                          (result, index) => (
                                            ...
                                            >
                                              <p className="font-medium text-slate-700 dark:text-slate-200 whitespace-pre-line">
                                                {formatLabResultTextWithReference(
                                                  result,
                                                )}
                                                {result.verified
                                                  ? " • Validé"
...
function PatientList({
  ...
  onSelect: (patient: DoctorPatient) => void;
}) {
  return (
    <aside className="rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
      <input
        ...
        className="mb-4 w-full rounded-lg border border-slate-200 px-4 py-3 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white"
      />{" "}
      <div className="space-y-3">
        {patients.map((patient) => (
          <button
            ...
            className={`w-full rounded-lg border p-3 text-left ${selectedId === patient.id ? "border-blue-300 bg-blue-50" : "border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950"}`}
          >
            <p className="font-semibold text-slate-900 dark:text-white">
...
function PatientHeader({ patient }: { patient: DoctorPatient }) {
  return (
    <div className="flex flex-col gap-2 border-b border-slate-200 pb-4 dark:border-slate-800">
      ...
      <p className="text-sm text-slate-500">
        {serviceLabel(patient)} - {patient.workflowStatus}
      </p>
      <span
        className={`w-fit rounded-full px-3 py-1 text-xs font-semibold ${patient.access?.canWrite ? "bg-emerald-100 text-emerald-700" : "bg-blue-100 text-blue-700"}`}
      >
        {patient.access?.canWrite ? "Ecriture autorisee" : "Lecture seule"}
      </span>
    </div>
  );
}
  ...
  return (
    <label className="block text-sm">
      ...
      <textarea
        ...
        onChange={(event) => onChange(event.target.value)}
        rows={3}
        className="w-full resize-none rounded-lg border border-slate-200 px-3 py-2 dark:border-slate-700 dark:bg-slate-950 dark:text-white"
      />
    </label>
  );
}
...
function EmptyState() {
  return (
    <div className="mt-6 rounded-lg border border-slate-200 bg-white p-8 text-center text-sm text-slate-500">
      Aucune consultation disponible. Cet onglet apparait utilement lorsqu'au
      moins une consultation existe.
    </div>
  );
}
...
function SmallEmpty() {
  return (
    <p className="rounded-lg bg-slate-50 p-3 text-sm text-slate-500 dark:bg-slate-950">
