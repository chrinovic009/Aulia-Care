...
export default function PrescriptionsMedecin() {
  ...
  const load = async () => {
    const [patientData, medicationData] = await Promise.all([
      fetchDoctorVisiblePatients(),
      fetchAvailableMedications().catch(() => []),
    ]);
  ...
  const selectableMedications = useMemo(() => {
    ...
    return medications.filter((item) => {
      const matchesSection =
        !sectionId || item.category?.section?.id === sectionId;
      const matchesCategory = !categoryId || item.category?.id === categoryId;
      const matchesSearch =
        !normalized ||
        `${item.name} ${item.strength || ""} ${item.code}`
          .toLowerCase()
          .includes(normalized);
  ...
  const pendingExam = Boolean(
    selectedPatient?.labRequests?.some((request) => {
      ...
      const hasVerifiedResult = request.results?.some(
  ...
  const updateMedicationDetail = (
    medicationId: string,
    patch: Partial<(typeof selectedMedicationDetails)[string]>,
  ) => {
    setSelectedMedicationDetails((current) => ({
      ...current,
      [medicationId]: {
  ...
  const submit = async () => {
    ...
    if (pendingExam) {
      setMessage(
        "Prescription verrouillee: un resultat d'examen demande n'est pas encore verifie.",
      );
      return;
    }
  ...
  const canModifyPrescription = (
    ...
    const prescriptionAge =
      Date.now() - new Date(prescription.prescribingDate).getTime();
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 dark:bg-slate-950 sm:p-6">
      <PageMeta
        title="Prescriptions medecin | Aulia Care"
        description="Prescriptions depuis le stock pharmaceutique."
      />
      <PageBreadcrumb pageTitle="Prescriptions" />
      <Header
        title="Prescriptions"
        subtitle="Prescrire depuis les medicaments disponibles au stock, apres resultats d'examens si necessaire."
      />
      {patients.length === 0 ? (
        <EmptyState />
      ) : (
                  ...
                  {false && <Panel title="Nouvelle prescription">
                    ...
                    <Select
                      ...
                      options={[
                        ...
                        ...sections.map(
                          (section) =>
                            [section.id, section.name] as [string, string],
                        ),
                      ]}
                    />
                    <Select
                    ...
                    <Input
                      ...
                      placeholder="Tapez un nom, une dose ou un code…"
                    />
                    {searchTerm.trim() ? (
                      <div className="max-h-52 overflow-y-auto rounded-xl border border-aulia-teal/25 bg-white p-1 shadow-sm dark:bg-slate-950">
                        {selectableMedications.length ? (
                          selectableMedications
                            .slice(0, 10)
                            .map((medication) => (
                              <button
                                type="button"
                                key={medication.id}
                                onClick={() =>
                                  addMedicationSelection(medication.id)
                                }
                                className="flex w-full items-center justify-between gap-3 rounded-lg px-3 py-2 text-left hover:bg-aulia-mist dark:hover:bg-aulia-teal/10"
                              >
                                <span>
                                  ...
                                  <span className="block text-xs text-slate-500">
                                    {medication.category?.section?.name ||
                                      "Sans section"}{" "}
                                    ·{" "}
                                ...
                                <span className="shrink-0 text-xs font-medium text-aulia-teal">
                      ...
                      >
                        ...
                        {selectableMedications.map((medication) => (
                          <option
                            key={medication.id}
                            value={medication.id}
                          >{`${medication.name}${medication.strength ? ` ${medication.strength}` : ""} - stock ${medication.availableQuantity}`}</option>
                        ))}
                      </select>
                                ...
                                <div className="mt-2 grid gap-2 md:grid-cols-2">
                                  <label className="block text-xs">
                                    ...
                                    <input
                                      ...
                                      min="1"
                                      value={
                                        selectedMedicationDetails[medication.id]
                                          ?.quantity ?? "1"
                                      }
                                      onChange={(event) =>
                                        updateMedicationDetail(medication.id, {
                                          quantity: event.target.value,
                                        })
                                      }
                                      className="w-full rounded-lg border border-slate-200 px-2 py-2 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white"
                                    />
                                  </label>
                                  <label className="block text-xs">
                                    <span className="mb-1 block font-medium text-slate-600 dark:text-slate-300">
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                      ...
                      <Input
                        ...
                        value={form.dosage}
                        onChange={(value) =>
                          setForm((current) => ({ ...current, dosage: value }))
                        }
                      />
                      <Select
                        label="Voie"
                    ...
                    <button
                      ...
                      onClick={submit}
                      className="rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white disabled:bg-slate-300 disabled:text-slate-600"
                    >
                      Prescrire
                    </button>
                  </Panel>}
                            ...
                            <div className="flex items-start justify-between gap-3">
                              <div>
                                <p className="font-semibold text-slate-900 dark:text-white">
                                  ...
                                  -{" "}
                                  {prescription.prescriber?.displayName ||
                                    "Medecin"}
                                </p>
                                <p className="mt-1 text-xs text-slate-500">
                                  {formatDateTime(prescription.prescribingDate)}
                                </p>
                                <p className="mt-2 text-slate-600 dark:text-slate-300">
                                  {prescription.lineItems
                                    ?.map(
                                      (line) =>
                                        `${line.medication?.name || "Medicament"} - ${line.dosage || ""} - ${line.frequency || ""}`,
                                    )
                                    .join(", ") ||
    ...
    <div className="flex flex-col gap-2 border-b border-slate-200 pb-4 dark:border-slate-800">
      ...
      {pendingExam && (
        <span className="w-fit rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-700">
          Examen en attente: prescription bloquee
        </span>
      )}
    </div>
  );
