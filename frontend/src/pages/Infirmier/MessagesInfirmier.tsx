...
const Messages = () => {
  ...
  const state = location.state as { patientInfo?: string; patientId?: string } | undefined;
  ...
  // Si un patient a été passé via navigation, afficher la conversation directement
  useEffect(() => {
    if (patientInfo && !isDesktop) {
      setShowConversation(true);
    }
  }, [patientInfo, isDesktop]);
        ...
        <div className="grid gap-6 xl:grid-cols-[320px_1fr]">
          ...
          {(isDesktop || showConversation) && (
            <div className="flex min-h-[640px] flex-col rounded-3xl border border-gray-200 bg-slate-50 dark:border-gray-800 dark:bg-slate-950">
              ...
              {patientInfo && (
                <div className="mb-4 rounded-3xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-950">
                  <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Infos du patient</p>
                  <p className="mt-2 whitespace-pre-wrap text-sm text-slate-700 dark:text-slate-200">{patientInfo}</p>
                </div>
              )}
              <div className="flex items-center gap-3">
                <input
