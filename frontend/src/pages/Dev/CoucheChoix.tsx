...
type ProvisioningFeedback = { kind: "success" | "error"; text: string } | null;
...
function provisioningErrorMessage(error: unknown) {
  ...
  return error instanceof Error ? error.message : "L’action n’a pas pu être effectuée.";
}
...
export default function CoucheChoix() {
  ...
  const [message, setMessage] = useState<ProvisioningFeedback>(null);
  ...
  const run = async (task: () => Promise<void>) => {
    ...
    try { await task(); } catch (error) { setMessage({ kind: "error", text: provisioningErrorMessage(error) }); }
  ...
  const createSuperAdmin = () => run(async () => {
    ...
    if (!superAdmin.firstName.trim() || !superAdmin.lastName.trim()) {
      throw new Error("Le prénom et le nom du Super Admin sont obligatoires.");
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      throw new Error("Saisissez une adresse e-mail valide pour le Super Admin.");
    }
    if (!/^[a-zA-Z0-9._-]{3,80}$/.test(username)) {
      throw new Error("L’identifiant doit contenir entre 3 et 80 caractères : lettres, chiffres, point, tiret ou souligné.");
    }
    if (superAdmin.password.length < 12) {
      throw new Error("Le mot de passe initial doit contenir au moins 12 caractères.");
    }
    await apiFetch(`/platform/provisioning/clinics/${provisioning.clinic.id}/super-admin`, { method: "POST", body: JSON.stringify({ ...superAdmin, email, username }) });
...
function Field({ label, value, onChange, type = "text", required, placeholder, maxLength, minLength }: { label: string; value: string; onChange: (value: string) => void; type?: string; required?: boolean; placeholder?: string; maxLength?: number; minLength?: number }) {
  return <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200"><span>{label}</span><input type={type} value={value} required={required} placeholder={placeholder} maxLength={maxLength} minLength={type === "password" ? Math.max(minLength ?? 0, 12) : minLength} onChange={(event) => onChange(event.target.value)} className="aulia-input mt-1.5" /></label>;
}
