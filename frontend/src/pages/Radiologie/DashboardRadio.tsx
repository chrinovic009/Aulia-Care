...
export default function DashboardRadio() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const response = await apiFetch<DashboardOverview>(
        `/imaging/dashboard/overview?period=${filters.period}&modality=${filters.modality}&service=${filters.service}`,
      );
      setData(response);
      setError(null);
    } catch (reason) {
      setError(
        reason instanceof Error
          ? reason.message
          : "Impossible de charger le tableau de bord radiologie.",
      );
    } finally {
      setLoading(false);
    }
    ...
    return [
      {
        ...
        value: data.metrics.totalScheduled,
        tone: "blue",
        description: `${data.metrics.realized} réalisés`,
      },
      {
        label: "Temps d’attente moyen",
        value: `${data.metrics.averageWaitMinutes} min`,
        tone: "amber",
        description: "Depuis l’arrivée et la préparation",
      },
      {
        label: "Durée moyenne",
        value: `${data.metrics.averageDurationMinutes} min`,
        tone: "emerald",
        description: "Examen réalisé",
      },
      {
        label: "Occupation du plateau",
        value: `${data.metrics.occupancyRate}%`,
        tone:
          data.metrics.occupancyRate >= 75
            ? "red"
            : data.metrics.occupancyRate >= 50
              ? "amber"
              : "green",
        description: "Taux d’usage réel",
      },
    ];
  }, [data]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 dark:bg-slate-950 sm:p-6">
      <PageMeta
        title="Radiologie | Dashboard"
        description="Tableau de bord RIS-PACS basé sur les données backend"
      />
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-sm uppercase tracking-[0.25em] text-slate-400">
              Aulia Care · Imagerie médicale
            </p>
            <h1 className="mt-2 text-2xl font-semibold">
              Dashboard Radiologie
            </h1>
            <p className="mt-2 text-sm text-slate-300">
              Vue en temps réel du flux des patients, des KPI et de l’état du
              parc d’équipements.
            </p>
          </div>
          <div className="flex items-center gap-2 rounded-full border border-slate-700 bg-slate-800/80 px-3 py-2 text-sm">
            <span
              className={`h-2.5 w-2.5 rounded-full ${isRealtime ? "bg-emerald-500" : "bg-slate-500"}`}
            />
            {isRealtime
      ...
      <section className="mt-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="flex items-end">
            <button
              onClick={() => setIsRealtime((current) => !current)}
              className={`w-full rounded-lg px-3 py-2 text-sm font-semibold ${isRealtime ? "bg-emerald-600 text-white" : "border border-slate-300 text-slate-700 dark:border-slate-700 dark:text-slate-200"}`}
            >
      ...
      {error ? (
        <div className="mt-4 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700 dark:border-red-800 dark:bg-red-950/30 dark:text-red-200">
          {error}
        </div>
      ) : null}
      ...
      {loading ? (
        ...
        <div className="mt-5 rounded-xl border border-slate-200 bg-white p-6 text-sm text-slate-600 dark:border-slate-800 dark:bg-slate-900">
        ...
        <>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 xl:col-span-2">
              <div className="flex items-center justify-between">
                ...
                <p className="text-sm text-slate-500">
                  Volume réel d’examens par heure
                </p>
              </div>
              <div className="mt-4 space-y-3">
                {data.hourlyActivity.length === 0 ? (
                  <EmptyState label="Aucune activité horaire trouvée pour cette sélection." />
                ) : (
                      ...
                      <div className="mt-2 flex gap-2">
                        <div className="h-2 flex-1 rounded-full bg-red-100">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, item.emergencies * 20)}%`,
                            }}
                          />
                        </div>
                        <div className="h-2 flex-1 rounded-full bg-amber-100">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, item.hospitalized * 20)}%`,
                            }}
                          />
                        </div>
                        <div className="h-2 flex-1 rounded-full bg-blue-100">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, item.ambulatory * 20)}%`,
                            }}
                          />
                        </div>
                      </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              ...
              <div className="mt-4 space-y-3">
                {data.modalityBreakdown.length === 0 ? (
                  ...
                  data.modalityBreakdown.map((entry) => (
                    <div
                      key={entry.modality}
                      className="rounded-xl border border-slate-200 p-3 dark:border-slate-800"
                    >
          ...
          <div className="mt-5 grid gap-4 xl:grid-cols-2">
                  ...
                  data.workflowAlerts.map((alert) => (
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span
                          className={`rounded-full px-2.5 py-1 text-xs font-semibold ${alert.severity === "CRITIQUE" ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}
                        >
                          {alert.severity}
                        </span>
                      </div>
                      <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
                        {alert.modality} · retard {alert.waitingMinutes} min
                      </p>
                    </div>
                  ))
                )}
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              <div className="flex items-center justify-between">
                ...
                <p className="text-sm text-slate-500">
                  Préparation / acquisition / validation
                </p>
              </div>
              <div className="mt-4 space-y-3">
                {data.activeQueue.length === 0 ? (
                  <EmptyState label="Aucune demande en cours." />
                ) : (
          ...
          <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div className="flex items-center justify-between">
              ...
              <p className="text-sm text-slate-500">
                Statuts réels de maintenance et d’utilisation
              </p>
            </div>
            <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {data.equipmentStatus.length === 0 ? (
                <EmptyState label="Aucun équipement répertorié." />
              ) : (
                data.equipmentStatus.map((machine) => (
                  <div
                    key={machine.id}
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-semibold ${machine.isOperational ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}
                      >
                        {machine.status}
                      </span>
                    </div>
                    <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
                      Salle : {machine.roomNumber || "—"}
                    </p>
                    <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
                      Alertes : {machine.alertCount}
                    </p>
                  </div>
                ))
              )}
