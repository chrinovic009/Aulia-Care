        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-300">
              Supervision du parc radiologique, alertes et interventions
              biomédicales.
            </p>
          </div>
          <button
      ...
      <section className="mt-5 rounded-xl bg-white p-6 shadow-sm dark:bg-slate-900">
        ...
        {machines.length === 0 ? (
          <p className="mt-4 text-sm text-slate-500 dark:text-slate-400">
            Aucun équipement radiologique répertorié.
          </p>
        ) : (
            ...
            {machines.map((machine) => (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
                      {machine.roomNumber || "Localisation non renseignée"}
                    </p>
                  </div>
                  <span
                    className={`rounded-full px-2.5 py-1 text-xs font-semibold ${machine.isOperational ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300" : "bg-red-100 text-rose-700 dark:bg-rose-900/40 dark:text-rose-300"}`}
                  >
                ...
                <p className="mt-4 text-xs text-slate-500 dark:text-slate-400">
                  Mis à jour le{" "}
                  {new Date(machine.updatedAt).toLocaleDateString("fr-FR")} à{" "}
                  {new Date(machine.updatedAt).toLocaleTimeString("fr-FR")}
                </p>
              </div>
            ))}
          </div>
